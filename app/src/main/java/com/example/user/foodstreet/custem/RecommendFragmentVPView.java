...
import com.example.user.foodstreet.adapter.RecommendFragmentVpAdapter;
...
//            Log.e(TAG, "onResponse: " + recommendFoodByJson);
            List<RecommendFoodModel.ObjEntity.SanCanEntity> san_can = recommendFoodByJson.getObj().getSan_can();
            List<RecommendFoodModel.ObjEntity.SanCanTitlesEntity> san_can_titles = recommendFoodByJson.getObj().getSan_can_titles();
//            Log.e(TAG, "onResponse: "+san_can);
            ...
            for (int k=0;k<san_can.size()/3;k++){
                ArrayList<RecommendFoodModel.ObjEntity.SanCanEntity> dataList=new ArrayList<>();
                i=i+3;
                for (int j=i;j<i+3;j++){
...
//            Log.e(TAG, "onResponse:size= " + san_can_titles.size());
            for (int k=0;k<san_can_titles.size();k++) {
                ArrayList<RecommendFoodModel.ObjEntity.SanCanTitlesEntity> datas=new ArrayList<>();
                y=y+1;
                for (int t = y; t < y+1; t++) {
